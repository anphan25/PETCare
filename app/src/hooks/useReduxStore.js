/**
 * Redux-backed hooks that provide the same API as the old Zustand / custom hooks.
 * This file is the single import point for all store-related hooks.
 */
...
export function useCart() {
  ...
  return {
    ...
    updateQuantity: useCallback(
      (productId, quantity) => dispatch(updateQtyAction({ productId, quantity })),
      [dispatch]
    ),
...
import {
  ...
  removeBooking as removeBookingAction,
  selectBookings,
...
export function useWishlistStore() {
  ...
  // Convert array to Set for backward compatibility with components that use Set methods
...
// ── Mascot ─────────────────────────────────────────────────────────────
import {
  setWagging as setWaggingAction,
  triggerJump as triggerJumpThunk,
  selectIsWagging,
  selectIsJumping,
