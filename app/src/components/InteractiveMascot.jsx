...
function StyledVoxelDog(props) {
  ...
  useEffect(() => {
    const handleMouseMove = (event) => {
      // Vị trí cố định của Mascot: left-4 (16px), bottom-4 (16px), w=300px, h=350px
      const dogCenterX = 166; 
      const dogCenterY = window.innerHeight - 191;
      mouse.current.x = event.clientX - dogCenterX;
      mouse.current.y = event.clientY - dogCenterY;
    };
...
export default function InteractiveMascot() {
  return (
    <div className="fixed bottom-4 left-4 w-[300px] h-[350px] z-[9999] pointer-events-none">
      <Canvas camera={{ position: [0, 1.5, 5], fov: 45 }} gl={{ alpha: true }}>
        <ambientLight intensity={1.5} color="#ffffff" />
        {/* Thêm một đèn chiếu thẳng từ phía trước để sáng chó hơn */}
        <directionalLight position={[0, 2, 5]} intensity={2.0} color="#ffffff" />
        {/* Căn chỉnh lại hướng sáng để hợp với Glassmorphism */}
        ...
        {/* Xoay ngang sang phải (rotation=[0, Math.PI / 5, 0]) */}
        <ErrorBoundary fallback={<StyledVoxelDog position={[0, -0.8, 0]} scale={1.2} rotation={[0, Math.PI / 5, 0]} />}>
          <Suspense fallback={<StyledVoxelDog position={[0, -0.8, 0]} scale={1.2} rotation={[0, Math.PI / 5, 0]} />}>
            <DogModel position={[0, -0.8, 0]} scale={1.2} rotation={[0, Math.PI / 5, 0]} />
          </Suspense>
        </ErrorBoundary>
      </Canvas>
    </div>
