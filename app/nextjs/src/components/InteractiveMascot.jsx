...
import { useMascotStore } from '@/hooks/useReduxStore';
...
function StyledVoxelDog(props) {
  ...
  const { isJumping } = useMascotStore();
  ...
  return (
    <group ref={group} {...props}>
      <mesh position={[0, -0.2, 0]}><boxGeometry args={[0.9, 0.8, 1.4]} /><meshStandardMaterial color="#F9F9F9" roughness={0.6} /></mesh>
      <mesh position={[0, -0.3, 0.05]}><boxGeometry args={[0.92, 0.6, 1.3]} /><meshStandardMaterial color="#EDF1D6" roughness={0.8} /></mesh>
      <mesh position={[0, 0.25, 0.6]}><boxGeometry args={[0.7, 0.15, 0.7]} /><meshStandardMaterial color="#E49393" roughness={0.4} /></mesh>
      <group ref={headRef} position={[0, 0.6, 0.7]}>
        <mesh position={[0, 0, 0]}><boxGeometry args={[0.85, 0.8, 0.8]} /><meshStandardMaterial color="#F9F9F9" roughness={0.6} /></mesh>
        <mesh position={[0, -0.1, 0.45]}><boxGeometry args={[0.5, 0.35, 0.4]} /><meshStandardMaterial color="#EDF1D6" roughness={0.7} /></mesh>
        <mesh position={[0, 0.05, 0.66]}><boxGeometry args={[0.15, 0.1, 0.1]} /><meshStandardMaterial color="#222222" roughness={0.5} /></mesh>
        <mesh position={[-0.25, 0.15, 0.41]}><boxGeometry args={[0.1, 0.1, 0.1]} /><meshStandardMaterial color="#222222" /></mesh>
        <mesh position={[0.25, 0.15, 0.41]}><boxGeometry args={[0.1, 0.1, 0.1]} /><meshStandardMaterial color="#222222" /></mesh>
        <mesh position={[-0.4, 0.35, -0.1]} rotation={[0, 0, 0.2]}><boxGeometry args={[0.2, 0.4, 0.3]} /><meshStandardMaterial color="#9DC08B" roughness={0.8} /></mesh>
        <mesh position={[0.4, 0.35, -0.1]} rotation={[0, 0, -0.2]}><boxGeometry args={[0.2, 0.4, 0.3]} /><meshStandardMaterial color="#9DC08B" roughness={0.8} /></mesh>
      </group>
  ...
  return (
    ...
    >
      <Canvas camera={{ position: [0, 1.5, 5], fov: 45 }} gl={{ alpha: true }}>
        ...
        <StyledVoxelDog getCenter={getMascotCenter} position={[0, -0.8, 0]} scale={1.2} rotation={[0, -Math.PI / 5, 0]} />
      </Canvas>
    </div>
  );
}
