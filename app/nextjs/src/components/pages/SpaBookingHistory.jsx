    ...
    const fetchBookings = async () => {
      try {
        ...
        const formattedBookings = (data || []).map(booking => {
          ...
          const petName =
            booking.pet_id === '00000000-0000-0000-0000-000000000001'
              ? 'Buddy'
              : booking.pet_id === '00000000-0000-0000-0000-000000000002'
              ? 'Mochi'
              : 'My Pet';

          // Parse time from note field (format: "Time: HH:MM AM/PM")
          const timeMatch = booking.note?.match(/Time:\s*(.+)/);
    ...
    <div className="mesh-gradient min-h-screen relative text-charcoal selection:bg-sage/30">
      ...
      <main className="relative z-10 pt-32 pb-20 px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto">
        <header className="mb-16">
          <Link href="/" className="w-10 h-10 rounded-full glass-panel flex items-center justify-center text-sage-dark mb-8 hover:bg-white/40 transition-all hover:-translate-x-1">
        ...
        <div className="space-y-6">
          {showLoader ? (
              ...
              >
                ...
                <div className="flex items-center gap-6 relative z-10 w-full md:w-auto">
                  <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-3xl overflow-hidden shadow-2xl border-2 border-white/60 bg-white/30 shrink-0 group-hover:scale-105 transition-transform duration-700">
                    <img src={booking.image} alt="Spa" className="w-full h-full object-cover" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-2">
                      ...
                      <span className="text-[10px] uppercase font-bold text-surface-variant/80 tracking-widest bg-white/40 px-3 py-0.5 rounded-full border border-white/60">{booking.date}{booking.time ? ` · ${booking.time}` : ''}</span>
                    </div>
                    <div className="flex flex-wrap gap-1.5 mb-2">
                      {booking.services.split(',').map((s, i) => (
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="glass-panel p-16 rounded-3xl text-center border border-white/40">
              ...
              <Link href="/spa" className="btn-primary inline-flex items-center gap-2 px-8 py-4">
                Book Spa
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="relative h-52 overflow-hidden">
                <img src={selectedBooking.image} alt="Spa" className="w-full h-full object-cover" />
              ...
              <div className="p-7">
                ...
                <div className="mb-5">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedBooking.services.split(',').map((s, i) => (
                      <span key={i} className="text-xs font-bold text-sage-dark bg-sage/15 border border-sage/25 px-3 py-1.5 rounded-full">
                        {s.trim()}
                      </span>
                    ))}
                  </div>
                ...
                <div className="grid grid-cols-2 gap-3 mb-6">
                  <div className="bg-white/60 rounded-2xl p-4 border border-white/60">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="material-symbols-outlined text-sm text-sage-dark">calendar_month</span>
                      <span className="text-[10px] uppercase tracking-widest font-bold text-surface-variant">Date</span>
                    </div>
                    <p className="font-black text-forest text-sm leading-tight">{selectedBooking.date}</p>
                  </div>
                  <div className="bg-white/60 rounded-2xl p-4 border border-white/60">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="material-symbols-outlined text-sm text-sage-dark">schedule</span>
                      <span className="text-[10px] uppercase tracking-widest font-bold text-surface-variant">Time</span>
                    </div>
                    <p className="font-black text-forest text-sm leading-tight">{selectedBooking.time || '–'}</p>
                  </div>
                </div>
